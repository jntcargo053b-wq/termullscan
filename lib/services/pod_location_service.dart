...
class PodLocationService {
  ...
  // TIDAK mengekspos kontrol `setWaitForAccurateLocation` /
  // `PRIORITY_HIGH_ACCURACY` LocationRequest.Builder secara langsung —
  // native bridge ini dipertahankan untuk kontrol penuh atas parameter
  // request (interval, minUpdateInterval). `waitForAccurateLocation`
  // sendiri sekarang di-set false (lihat FusedLocationStreamHandler)
  // supaya callback pertama tidak ditahan FLP demi lock cepat — fix
  // kasar (network/cell-based) yang mungkin lolos sebagai sample
  // pertama tetap tersaring aman di _window lewat gate accuracy
  // adaptif PodGpsEngine (outdoorAccuracyThreshold/indoorAccuracyThreshold)
  // + MAD-based outlier rejection, bukan lagi dicegah di level native.
  //
