...
// VERSI DENGAN LOGGING & FALLBACK DRAWTEXT
import 'dart:io';
...
class VideoWatermarkService {
  ...
  static Future<String?> addWatermark({
    ...
    try {
      ...
      debugPrint('📹 Input video: ${videoInfo.width}x${videoInfo.height}, rotasi ${videoInfo.rotation}°, durasi ${videoInfo.duration}s');

      // 2. Hitung dimensi output & scale filter (resolusi selalu original)
      final dims = _computeDimensions(videoInfo);
      debugPrint('📐 Output: ${dims.outW}x${dims.outH} | Rotasi: ${dims.rotation}°');
      ...
      if (overlayResult == null) {
        ...
        // Fallback: gunakan drawtext langsung
      ...
      debugPrint('🎬 FFmpeg args (overlay): ${args.join(' ')}');
      ...
      if (!success) {
        // Gagal, coba fallback drawtext
        debugPrint('⚠️ Encoding dengan overlay gagal, coba drawtext fallback...');
        final fallbackResult = await _addWatermarkWithDrawtext(
          inputPath: inputPath,
          outputPath: outputPath,
          entry: entry,
  ...
  static Future<String?> _addWatermarkWithDrawtext({
    ...
    try {
      ...
      // Logo tidak bisa di-draw dengan drawtext, kita abaikan logo di fallback
      final command =
          "-i '$inputPath' "
          "-vf \"$drawText\" "
